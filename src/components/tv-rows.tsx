...
import { fetchTVRequest, MovieResponse, MovieResult } from "../common/api";
...
export default function TvRows({ title, endpoint }: RowProps) {
  ...
  async function fetchRowData() {
    const response = await fetchTVRequest<MovieResponse<MovieResult[]>>(
      endpoint
    );
      ...
      >
        ...
        >
          {rowData?.map((row, index) => {
            return <TvCard {...row} key={`${row.id}-${index}`} index={index} />;
          })}
        </section>
      </section>
    </section>
